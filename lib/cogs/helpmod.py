...
class HelpMenu(ListPageSource):
	...
	async def write_page(self, menu, fields=[]):
		...
		embed= Embed(title=f"Moderation Help",
    		description=f"Welcome to the **CHΛD丨BӨT** Moderation Help dialog. React to the arrows below in order to navigate through the panel!",
    		color=embed_color)
...
class Helpmod(Cog):
	...
	@command(name="helpmod", brief="Help For Moderation Commands", help="Provides Help For Moderation Commands", hidden=True)
	...
	async def on_ready(self):
		if not self.bot.ready:
			self.bot.cogs_ready.ready_up("helpmod")
